...
def doEpoch(results, best_model, epoch, criterion, optimizer, lr_scheduler, num_epochs):
    ...
    for phase in ['train', 'test']:
        ...
        TCat = confusion_matrix.value()[0][0]
        FDog = confusion_matrix.value()[0][1]
        FCat = confusion_matrix.value()[1][0]
        TDog = confusion_matrix.value()[1][1]
        ...
        print('True cat: {}, False cat: {}, True dog: {}, False dog: {}, TPR: {}, FPR: {}'.format(TCat, FCat, TDog, FDog, TPR, FPR))

        results = ('{} Loss: {:.8f} Acc: {:.8f}\n'.format(phase, epoch_loss, epoch_acc)) + ('True cat: {}, False cat: {}, True dog: {}, False dog: {}, TPR: {}, FPR: {}'.format(TCat, FCat, TDog, FDog, TPR, FPR)) + '\n'
        with open('results/species/model__species__Epoch__ ' + str(num_epochs) + '__LR__' + str(LR) + '.txt', 'a') as f:
            f.write(results)
        f.close
...
torch.save(model.cpu().state_dict(), 'results/species/build_model_species.pkl')
